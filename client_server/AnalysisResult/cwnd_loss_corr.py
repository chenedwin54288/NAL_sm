...
OUTPUT_DIR = Path(__file__).resolve().parent / "CwndLoss"
...
def read_loss_rate(database_path: Path) -> float:
    ip_info_path = database_path / "ip_info.json"
    with ip_info_path.open("r", encoding="utf-8") as file:
        ip_info = json.load(file)

    row_count = ip_info["row_count"]
    if row_count == 0:
...
def collect_points(database_paths: list[str]) -> tuple[str, list[tuple[int, float]]]:
    ...
    for path_string in database_paths:
        ...
        points.append((extract_cwnd(database_path), read_loss_rate(database_path)))
...
def plot_cwnd_loss_correlation(data_sent_size: str, points: list[tuple[int, float]]) -> Path:
    cwnds = [cwnd for cwnd, _ in points]
    loss_rates = [loss_rate for _, loss_rate in points]

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    output_path = (
        OUTPUT_DIR
        / f"cwnd_loss_corr_{data_sent_size}_{min(cwnds)}_{max(cwnds)}.png"
    )

    plt.figure(figsize=(8, 5))
    plt.scatter(cwnds, loss_rates, s=70)
    plt.xlabel("cwnd")
    plt.ylabel("Loss rate (loss_recovery / row_count)")
    plt.title(f"cwnd vs loss rate ({data_sent_size})")
...
def main() -> None:
    ...
    output_path = plot_cwnd_loss_correlation(data_sent_size, points)
