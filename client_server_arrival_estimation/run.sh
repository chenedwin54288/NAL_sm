...
Options:
  ...
  --cumulative-probe-format M Cumulative probe format: framed or raw
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --cumulative-probe-format)
      CUMULATIVE_PROBE_FORMAT="${2:-}"; shift 2 ;;
    --no-cumulative)
...
if [[ -n "$CUMULATIVE_PROBE_FORMAT" ]]; then
  [[ "$CUMULATIVE_PROBE_FORMAT" == "framed" || "$CUMULATIVE_PROBE_FORMAT" == "raw" ]] || \
    die "--cumulative-probe-format must be framed or raw"
fi
...
if [[ "$CUMULATIVE_ENABLED" == "1" ]]; then
  ...
  if [[ -n "$CUMULATIVE_PROBE_FORMAT" ]]; then
    CUMULATIVE_CMD+=(--probe-format "$CUMULATIVE_PROBE_FORMAT")
  fi
